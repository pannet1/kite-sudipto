...
class KiteApp:
    ...
    def instruments(self, exchange=None):
        ...
        for i in data[1:-1]:
            ...
            if exchange is None or exchange == row[11]:
                Exchange.append({'instrument_token':int(row[0]),  'exchange_token':row[1],  'tradingsymbol':row[2],  'name':row[3][1:-1], 
                 'last_price':float(row[4]),  'expiry':dateutil.parser.parse(row[5]).date() if row[5] != '' else None, 
                 'strike':float(row[6]), 
                 'tick_size':float(row[7]),  'lot_size':int(row[8]),  'instrument_type':row[9], 
                 'segment':row[10],  'exchange':row[11]})
    ...
    def historical_data(self, instrument_token, from_date, to_date, interval, continuous=False, oi=False):
        params = {'from':from_date, 
         ...
         'oi':1 if oi else 0}
        lst = self.session.get(f"{self.root_url}/instruments/historical/{instrument_token}/{interval}",
          params=params, headers=(self.headers)).json()['data']['candles']
        records = []
        for i in lst:
            record = {'date':dateutil.parser.parse(i[0]), 
             'open':i[1],  'high':i[2],  'low':i[3],  'close':i[4], 
             'volume':i[5]}
            if len(i) == 7:
                record['oi'] = i[6]
    ...
    def cancel_order(self, variety, order_id, parent_order_id=None):
        order_id = self.session.delete(f"{self.root_url}/orders/{variety}/{order_id}", data=({'parent_order_id': parent_order_id} if parent_order_id else {}),
          headers=(self.headers)).json()['data']['order_id']
        return order_id
